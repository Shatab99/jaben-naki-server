...
import { TRidePosts } from "../RidePosts/RidePosts.interface";
...
const bookRide = catchAsync(async (req, res) => {
    ...
    const { passengerEmail, passengerName, numberOfSeats } = req.body;
    ...
    const isBookExists = await bookRideModel.findOne({ passengerEmail })

    if (isBookExists) {
        throw new Error("You have already booked!!")
    }

    const passengerData = await PassengerModel.findOne({ email: passengerEmail })
    ...
    const result = await bookRideModel.create({
        ...
        passengerEmail, passengerName, numberOfSeats,
